...
docker run -d -h "${ZMS_DB_HOST}" \
    ...
    --name "${ZMS_DB_HOST}" athenz/athenz-zms-db:1.9.27
...
docker run --rm \
    ...
    --name wait-for-mysql athenz/athenz-zms-db:1.9.27 \
    --user='root' \
    --host="${ZMS_DB_HOST}" \
    --port=3306
...
docker run -d -h "${ZMS_HOST}" \
    ...
    -e "ZMS_PORT=${ZMS_PORT}" \
    --name "${ZMS_HOST}" athenz/athenz-zms-server:1.9.27
# wait for ZMS to be ready
until docker run --rm --entrypoint curl \
    --network="${DOCKER_NETWORK}" \
    --user "$(id -u):$(id -g)" \
    --name athenz-curl athenz/athenz-setup-env:1.9.27 \
