...
public class JDBCConnection implements ObjectStoreConnection {
    ...
    private static final String SQL_INSERT_SERVICE = "INSERT INTO service "
            ...
            + "x509_cert_signer_keyid, ssh_cert_signer_keyid, client_id) VALUES (?,?,?,?,?,?,?,?,?,?);";
    private static final String SQL_UPDATE_SERVICE = "UPDATE service SET modified=CURRENT_TIMESTAMP(3), "
            ...
            + "x509_cert_signer_keyid=?, ssh_cert_signer_keyid=?, creds=?, client_id=? WHERE service_id=?;";
    ...
    boolean validatePrincipalDomain(String principal) {
        ...
        // special handling for group and external principals
        int idx = principal.indexOf(AuthorityConsts.ATHENZ_PRINCIPAL_ENTITY_CHAR);
        if (idx != -1) {
            if (!principal.regionMatches(idx, AuthorityConsts.GROUP_SEP, 0, AuthorityConsts.GROUP_SEP.length()) &&
                !principal.regionMatches(idx, AuthorityConsts.EXT_SEP, 0, AuthorityConsts.EXT_SEP.length())) {
                idx = -1;
            }
        }
        if (idx == -1) {
