...
public class KmsCaCertificateStoreTest {
    ...
    public void testMissingCertificateFile() throws Exception {
        ...
        Files.writeString(invalidPem.toPath(),
                "-----BEGIN CERTIFICATE-----\n@@@@\n-----END CERTIFICATE-----\n");
        Files.setLastModifiedTime(invalidPem.toPath(),
                FileTime.fromMillis(Files.getLastModifiedTime(invalidPem.toPath()).toMillis() + 1000));
        expectThrows(CrypkiException.class,
                () -> new KmsCaCertificateStore(invalidPem.getAbsolutePath(), null).get("alias/x"));
    }
    ...
    @Test
    public void testReloadsCertificateWhenFileChanges() throws Exception {
        ...
        Files.setLastModifiedTime(Path.of(path),
                FileTime.fromMillis(Files.getLastModifiedTime(Path.of(path)).toMillis() + 1000));
        assertEquals(store.get(null).getSubjectX500Principal(), second.getSubjectX500Principal());
        expectThrows(CrypkiException.class, () -> KmsCaCertificateStore.fileIdentity("/missing-ca.pem"));
        KmsCaCertificateStore.FileIdentity identity = KmsCaCertificateStore.fileIdentity(path);
        assertTrue(identity.sameAs(identity));
        assertTrue(!identity.sameAs(null));
        assertTrue(!identity.sameAs(new KmsCaCertificateStore.FileIdentity(identity.lastModified,
                identity.size + 1)));
        assertTrue(!identity.sameAs(new KmsCaCertificateStore.FileIdentity(
                FileTime.fromMillis(identity.lastModified.toMillis() + 1000), identity.size)));
    }
