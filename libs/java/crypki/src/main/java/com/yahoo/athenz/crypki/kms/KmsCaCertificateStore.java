 ...
 * {@code x509CertSignerKeyId}. Domain/service signer ids are CompoundNames
 * ({@code tenant-a-ca}); they cannot be {@code alias/...} or GCP resource
 * names. KMS and CloudHSM share this JSON shape. A single default path
...
public class KmsCaCertificateStore {
    ...
    private final Map<String, CachedCertificate> cache = new ConcurrentHashMap<>();
    ...
    X509Certificate certificate(String certPath) {
        FileIdentity identity = fileIdentity(certPath);
        CachedCertificate cached = cache.get(certPath);
        if (cached != null && cached.identity.sameAs(identity)) {
            return cached.certificate;
        }
        X509Certificate loaded = loadCertificate(certPath);
        cache.put(certPath, new CachedCertificate(loaded, identity));
        return loaded;
    }
    ...
    static FileIdentity fileIdentity(String certPath) {
        try {
            BasicFileAttributes attributes = Files.readAttributes(Path.of(certPath),
                    BasicFileAttributes.class);
            return new FileIdentity(attributes.lastModifiedTime(), attributes.size());
        } catch (Exception ex) {
    ...
    static final class CachedCertificate {
        final X509Certificate certificate;
        final FileIdentity identity;

        CachedCertificate(X509Certificate certificate, FileIdentity identity) {
    ...
    static final class FileIdentity {
        ...
        FileIdentity(FileTime lastModified, long size) {
