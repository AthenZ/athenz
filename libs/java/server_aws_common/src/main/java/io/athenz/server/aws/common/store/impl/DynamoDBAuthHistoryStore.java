...
public class DynamoDBAuthHistoryStore implements AuthHistoryStore {
    ...
    @Override
    public AuthHistoryStoreConnection getConnection() throws ServerResourceException {
        try {
            return new DynamoDBAuthHistoryStoreConnection(mappedTable);
        } catch (Exception ex) {
            LOGGER.error("getConnection: {}", ex.getMessage());
            throw new ServerResourceException(ServerResourceException.SERVICE_UNAVAILABLE, ex.getMessage());
        }
    }
