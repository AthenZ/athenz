...
public class AwsCrypkiSignerFactoryTest {
    ...
    public void resetCloudHsmStubs() {
        ...
        CloudHsmProvider.failLoginAccountAlready = false;
        CloudHsmProvider.throwOnConstruct = false;
        CloudHsmProvider.loginCount = 0;
    ...
    public void testCloudHsmClientLoadsMappedTenantLabels() throws Exception {
        ...
        try {
            ...
            expectThrows(CrypkiException.class, () -> client.getSigningKey("other-label"));
            assertEquals(CloudHsmProvider.loginCount, 1);
        } finally {
    ...
    public void testCloudHsmJceProviderHelpers() throws Exception {
        ...
        CloudHsmProvider.failLoginAccountAlready = true;
        try {
            CloudHsmProvider alreadyLoggedIn = new CloudHsmProvider();
            AwsCloudHsmClient.loginCloudHsmJce(alreadyLoggedIn, "pin".toCharArray());
            assertTrue(alreadyLoggedIn.loggedIn);
        } finally {
            CloudHsmProvider.failLoginAccountAlready = false;
        }
        ...
        assertTrue(AwsCloudHsmClient.alreadyConnected(
                new CloudHsmProvider.AccountAlreadyLoggedInException("CU already logged in")));
        assertTrue(AwsCloudHsmClient.alreadyConnected(
                new javax.security.auth.login.LoginException("user already logged in")));
        ...
        assertTrue(!AwsCloudHsmClient.alreadyConnected(null));
        assertTrue(!AwsCloudHsmClient.alreadyConnected(
                new IllegalStateException("credentials already expired")));
        expectThrows(javax.security.auth.login.LoginException.class, () -> {
            CloudHsmProvider denied = new CloudHsmProvider();
            CloudHsmProvider.failLogin = true;
            try {
                AwsCloudHsmClient.loginCloudHsmJce(denied, "pin".toCharArray());
            } finally {
                CloudHsmProvider.failLogin = false;
            }
        });
        expectThrows(CrypkiException.class, () -> AwsCloudHsmClient.loadSigningKey(null,
                "/missing-module.so", null, "athenz-crypki-ca", "pin".toCharArray(), null));
        expectThrows(CrypkiException.class, () -> AwsCloudHsmClient.loadSigningKey(null,
                modulePathForCoverage(), null, "athenz-crypki-ca", "pin".toCharArray(),
                hsmCertificate("CN=hsm-ca")));
        CloudHsmProvider.failLogin = true;
        try {
            expectThrows(CrypkiException.class, () -> new AwsCloudHsmClient(modulePathForCoverage(),
                    null, "athenz-crypki-ca", pinFile("crypto-user:example-pin"), caCertFile()));
        } finally {
            CloudHsmProvider.failLogin = false;
        }
    }
