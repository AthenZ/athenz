...
public class DynamoDBAuthHistoryStoreConnectionTest {
    ...
    @Test
    public void testGetAuthHistory() throws ServerResourceException {
    ...
    public void testGetAuthHistoryException() {
        ...
        Mockito.when(principalDomainIndex.query(Mockito.any(Consumer.class)))
                .thenThrow(ResourceNotFoundException.builder().message("records do not exist").build());
        try {
            dynamoDBAuthHistoryStoreConnection.getAuthHistory("test.domain");
            fail();
        } catch (ServerResourceException resourceException) {
            assertEquals(resourceException.getMessage(), "records do not exist");
        }
    }
