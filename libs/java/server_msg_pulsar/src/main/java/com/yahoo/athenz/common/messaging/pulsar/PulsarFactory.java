...
public class PulsarFactory<T> implements ChangePublisherFactory<T>, ChangeSubscriberFactory<T> {
    ...
    @Override
    public ChangeSubscriber<T> create(PrivateKeyStore keyStore, String topicName, String subscriptionName,
            String subscriptionTypeAsString) {
        return new PulsarChangeSubscriber<>(serviceUrl(), topicName, subscriptionName,
                toSubscriptionType(subscriptionTypeAsString), tlsConfig());
    }
