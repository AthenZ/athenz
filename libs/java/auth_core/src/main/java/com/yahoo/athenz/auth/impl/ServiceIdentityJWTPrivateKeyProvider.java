...
public class ServiceIdentityJWTPrivateKeyProvider implements ServiceIdentityProvider {

    private static final Logger LOG = LoggerFactory.getLogger(ServiceIdentityJWTPrivateKeyProvider.class);
    ...
    final String privateKeyPemFile;
    final String privateKeyId;
    final int expiryTimeSecs;
    JWSAlgorithm jwsAlgorithm = null;

    /**
     * Constructs a new ServiceIdentityJWTPrivateKeyProvider. The provider can
     ...
     * @param privateKeyPemFile the path to the PEM-encoded private key file
     * @param privateKeyId the key ID to include in the JWT header
     */
    public ServiceIdentityJWTPrivateKeyProvider(final String clientId, final String audience, int expiryTimeSecs,
            final String privateKeyPemFile, final String privateKeyId) {
     ...
     *
     * @param jwsAlgorithm the JWS algorithm as a string (e.g., "RS256", "ES256")
     */
    ...
    public String getClientAssertionValue() {
        ...
        try {
            PrivateKey privateKey = Crypto.loadPrivateKey(new File(privateKeyPemFile));
            ...
            // use the caller's specified algorithm if one is provided. otherwise,
            // default to the appropriate algorithm based on the private key type

            JWSAlgorithm algorithm = jwsAlgorithm;
            if (algorithm == null) {
                algorithm = Crypto.RSA.equals(privateKey.getAlgorithm()) ? JWSAlgorithm.RS256 : JWSAlgorithm.ES256;
            }

            SignedJWT signedJWT = new SignedJWT(new JWSHeader.Builder(algorithm).keyID(privateKeyId).build(), claimsSet);
