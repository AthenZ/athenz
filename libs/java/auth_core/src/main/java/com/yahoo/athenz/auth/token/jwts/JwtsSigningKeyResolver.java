...
public class JwtsSigningKeyResolver implements SigningKeyResolver {
    ...
    void loadPublicKeysFromConfig() {
        ...
        if (confFileName == null || confFileName.isEmpty()) {
            LOGGER.info("No conf file configured for jwt public keys");
            return;
        }
        ...
        try {
            ...
            final ArrayList<PublicKeyEntry> ztsPublicKeys = conf.getZtsPublicKeys();
            if (ztsPublicKeys == null) {
                LOGGER.error("Conf file {} has no ZMS Public keys", confFileName);
                return;
            }
            for (PublicKeyEntry publicKey : ztsPublicKeys) {
