...
type identityCustomFormatter struct{}

func (f *identityCustomFormatter) FormatServiceURI(trustDomain, namespace, domain, service, workloadId string) string {
	return fmt.Sprintf("spiffe://custom.identity/ns/%s/sa/%s.%s", namespace, domain, service)
}

func (f *identityCustomFormatter) FormatRoleURI(trustDomain, domain, role string) string {
	return ""
}

func (f *identityCustomFormatter) FormatUserURI(trustDomain, namespace, principalName, deviceId string) string {
	return ""
}
...
func TestGenerateIdentityWithCustomSpiffeFormatter(test *testing.T) {
	// Mock the metadata endpoints
	router := http.NewServeMux()
	router.HandleFunc("POST /instance", func(w http.ResponseWriter, r *http.Request) {
		// decode the InstanceRegisterInformation JSON to extract the CSR, then
		// parse the CSR to verify the SPIFFE URI SAN was generated by the custom formatter
		var regInfo zts.InstanceRegisterInformation
		contents, _ := io.ReadAll(r.Body)
		if err := json.Unmarshal(contents, &regInfo); err == nil {
			block, _ := pem.Decode([]byte(regInfo.Csr))
			if block != nil {
				parsedCSR, err := x509.ParseCertificateRequest(block.Bytes)
				if err == nil {
					found := false
					for _, uri := range parsedCSR.URIs {
						if uri.String() == "spiffe://custom.identity/ns/default/sa/sports.api" {
							found = true
							break
						}
					}
					if !found {
						test.Errorf("custom spiffe uri not found in CSR SANs, got: %v", parsedCSR.URIs)
					}
				}
			}
		}
	...
	identity, err := RegisterIdentityWithSpiffeFormatter("sports", "api", "provider", metaServer.URL, "id-001", "attestation-data", "", []string{"athenz.io"}, csrSubjectFields, false, privateKey, &identityCustomFormatter{})
	if err != nil {
		test.Errorf("unable to register identity with custom formatter: %v\n", err)
		return
	}
	if identity.TLSCertificate.Certificate == nil {
		test.Errorf("no TLS certificate available in the response\n")
