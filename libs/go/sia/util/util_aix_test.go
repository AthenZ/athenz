...
func TestGetGroupGIDSystemGroup(t *testing.T) {
	// Test with the GID 0 group, which on AIX is "system" (there is no
	// "root" group as on Linux; root's primary group is "system").
	gid := GetGroupGID("system")
	if gid != 0 {
		t.Errorf("Expected GID 0 for system group, got=%d", gid)
		return
	}
}
