...
func TestGetGroupGIDWheelGroup(t *testing.T) {

	// Test with wheel group which typically exists on Darwin/macOS systems
	gid := GetGroupGID("wheel")
	// wheel group should exist on Darwin/macOS systems and typically has GID 0
	if gid == -1 {
		t.Errorf("Failed to get GID for wheel group")
		return
	}
	// Verify it's a valid GID (non-negative)
	if gid < 0 {
		t.Errorf("Got invalid GID for wheel group: %d", gid)
		return
	}
}
