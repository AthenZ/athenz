...
func GetMetaDetailsFromCreds(serviceSuffix string, useRegionalSTS bool, sysLogger io.Writer) (string, string, string, string, error) {
	...
	return parseRoleArn(*result.Arn, serviceSuffix, region)
}

func parseRoleArn(roleArn, serviceSuffix, region string) (string, string, string, string, error) {
	//arn:aws:sts::123456789012:assumed-role/athenz.zts-service/i-0662a0226f2d9dc2b
	if !strings.HasPrefix(roleArn, "arn:aws:sts:") {
		return "", "", "", region, fmt.Errorf("unable to parse role arn (prefix): %s", roleArn)
	}
	...
	if roleComps[0] != "assumed-role" {
		return "", "", "", region, fmt.Errorf("unable to parse role arn (assumed-role): %s", roleArn)
	}
	...
	if !strings.HasSuffix(roleComps[1], serviceSuffix) {
		return "", "", "", region, fmt.Errorf("service name does not have '%s' suffix: %s", serviceSuffix, roleArn)
	}
