...
func TestGetMetadataV1(test *testing.T) {
	...
	_, err := GetDataV1(ts.URL, "/latest/dynamic/instance-identity/document")
	if err != nil {
...
func TestGetMetadataV2(test *testing.T) {
	// Mock the metadata endpoints
	router := http.NewServeMux()
	router.HandleFunc("/latest/api/token", func(w http.ResponseWriter, r *http.Request) {
	...
	_, err := GetDataV2(ts.URL, "/latest/dynamic/instance-identity/document")
	if err != nil {
