	...
	if len(params) >= 1 {
		...
		case "show-resource":
			if argc >= 2 {
				...
				if argc == 3 {
					var err error
					isCaseSensitive, err = strconv.ParseBool(args[2])
					if err != nil {
						return nil, err
					}
				}
				return cli.ShowResourceAccess(args[0], args[1], &isCaseSensitive)
			}
		case "list-user":
		...
		case "show-access":
			if argc >= 2 {
				...
				if argc > 2 {
					...
					if argc > 4 {
						var err error
						isCaseSensitive, err = strconv.ParseBool(args[4])
						if err != nil {
							return nil, err
						}
					}
				}
				return cli.ShowAccess(dn, args[0], args[1], altPrincipal, trustDomain, &isCaseSensitive)
			}
		case "show-access-ext":
			if argc >= 2 {
				...
				return cli.ShowAccessExt(dn, args[0], args[1], altPrincipal, trustDomain, &isCaseSensitive)
			}
...
func (cli Zms) HelpSpecificCommand(interactive bool, cmd string) string {
	...
	case "set-domain-member-expiry-days":
		buf.WriteString(" syntax:\n")
		buf.WriteString("   " + domain_param + " set-domain-member-expiry-days days\n")
		buf.WriteString(" parameters:\n")
		if !interactive {
			buf.WriteString("   domain        : name of the domain being updated\n")
		}
	...
	case "set-domain-service-expiry-days":
		buf.WriteString(" syntax:\n")
		buf.WriteString("   " + domain_param + " set-domain-service-expiry-days days\n")
		buf.WriteString(" parameters:\n")
		if !interactive {
			buf.WriteString("   domain        : name of the domain being updated\n")
		}
		...
		buf.WriteString(" examples:\n")
		buf.WriteString("   " + domain_example + " set-domain-service-expiry-days 60\n")
	case "set-domain-service-cert-expiry-mins":
		buf.WriteString(" syntax:\n")
	...
	case "set-domain-token-expiry-mins":
		...
		buf.WriteString(" examples:\n")
		buf.WriteString("   " + domain_example + " set-domain-token-expiry-mins 1800\n")
	case "set-org-name":
		buf.WriteString(" syntax:\n")
	...
	case "show-access":
		buf.WriteString(" syntax:\n")
		buf.WriteString("   " + domain_param + " show-access action resource [alt_identity [trust_domain]] [is_case_sensitive]\n")
		buf.WriteString(" parameters:\n")
		if !interactive {
	...
	case "show-access-ext":
		buf.WriteString(" syntax:\n")
		buf.WriteString("   " + domain_param + " show-access-ext action resource [alt_identity [trust_domain]] [is_case_sensitive]\n")
		buf.WriteString(" parameters:\n")
		if !interactive {
			buf.WriteString("   domain       : name of the domain that resource belongs to\n")
		}
		...
		buf.WriteString("   " + domain_example + " show-access-ext NODE_SUDO coretech:node.HOST1 true\n")
		buf.WriteString("   " + domain_example + " show-access-ext node_sudo coretech:node.host1 " + cli.UserDomain + ".john\n")
	case "show-resource":
		buf.WriteString(" syntax:\n")
		buf.WriteString("   show-resource principal action [is_case_sensitive]\n")
		buf.WriteString(" parameters:\n")
		...
		buf.WriteString("   show-resource " + cli.UserDomain + ".user1 UPDATE true\n")
	...
	case "set-role-member-expiry-days":
		...
		buf.WriteString(" examples:\n")
		buf.WriteString("   " + domain_example + " set-role-member-expiry-days writers 60\n")
	case "set-role-service-expiry-days":
		buf.WriteString(" syntax:\n")
		buf.WriteString("   " + domain_param + " set-role-service-expiry-days role days\n")
		buf.WriteString(" parameters:\n")
		if !interactive {
			buf.WriteString("   domain  : name of the domain being updated\n")
		}
		...
		buf.WriteString(" examples:\n")
		buf.WriteString("   " + domain_example + " set-role-service-expiry-days writers 60\n")
	case "set-role-member-review-days":
		buf.WriteString(" syntax:\n")
		buf.WriteString("   " + domain_param + " set-role-member-review-days role days\n")
		buf.WriteString(" parameters:\n")
		if !interactive {
			buf.WriteString("   domain  : name of the domain being updated\n")
		}
		...
		buf.WriteString(" examples:\n")
		buf.WriteString("   " + domain_example + " set-role-member-review-days writers 60\n")
	case "set-role-service-review-days":
		buf.WriteString(" syntax:\n")
		buf.WriteString("   " + domain_param + " set-role-service-review-days role days\n")
		buf.WriteString(" parameters:\n")
		if !interactive {
			buf.WriteString("   domain  : name of the domain being updated\n")
		}
...
func (cli Zms) HelpListCommand() string {
	...
	buf.WriteString("   show-access action resource [alt_identity [trust_domain]] [is_case_sensitive]\n")
	buf.WriteString("   show-access-ext action resource [alt_identity [trust_domain]] [is_case_sensitive]\n")
	buf.WriteString("   show-resource principal action [is_case_sensitive]\n")
	buf.WriteString("\n")
