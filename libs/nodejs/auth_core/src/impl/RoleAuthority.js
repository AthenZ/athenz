  ...
  authenticate(signedToken, remoteAddr, httpMethod) {
    ...
    try {
      ...
      winston.error('PrincipalAuthority:authenticate: Invalid token: exc=' + e.message +
        ' : credential=' + RoleToken.getUnsignedToken(signedToken));
      return null;
    }
    ...
    if (remoteAddr !== roleToken.getIP() && this._isWriteOperation(httpMethod)) {
      ...
      if (tokenPrincipal.substring(0, idx).toLowerCase() === this._userDomain) {
        winston.error('RoleAuthority:authenticate failed: IP Mismatch - tokenip(' +
          roleToken.getIP() + ') request-addr(' +
          remoteAddr + ') credential=' +
          RoleToken.getUnsignedToken(signedToken));
        return null;
      }
    }

    var publicKey = this._keyStore.getPublicKey(SYS_AUTH_DOMAIN, ZTS_SERVICE, roleToken.getKeyId());

    if (roleToken.validate(publicKey, this._allowedOffset, false) === false) {
      winston.error('RoleAuthority:authenticate failed: validation was not successful: credential=' +
        RoleToken.getUnsignedToken(signedToken));
      return null;
    }
    ...
    var princ = SimplePrincipal.createByRoles(roleToken.getDomain(), signedToken, roleToken.getRoles(), this);
