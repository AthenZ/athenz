...
class SimpleServiceIdentityProvider {
  ...
  constructor(domainName, serviceName, privateKey, keyId) {
    ...
    this._domain = domainName.toLowerCase();
    this._service = serviceName.toLowerCase();
    this._key = privateKey;
    this._tokenTimeout = 3600;
    this._keyId = keyId.toLowerCase();
  ...
  getIdentity(domainName, serviceName) {
    ...
    var domainName = domainName.toLowerCase();
    var serviceName = serviceName.toLowerCase();
    ...
    var principal = SimplePrincipal.createByUserIdentity(domainName, serviceName, token.getSignedToken(), Date.now() / 1000, this._Authority);
