...
class PrincipalAuthority {
  constructor() {
    this._keyStore = null;
    this._allowedOffset = (ATHENZ_PROP_TOKEN_OFFSET !== null) ? Number(ATHENZ_PROP_TOKEN_OFFSET) : 300;
  ...
  authenticate(signedToken, remoteAddr, httpMethod) {
    ...
    var tokenDomain = serviceToken.getDomain().toLowerCase();
    var tokenName = serviceToken.getName().toLowerCase();
  ...
  _isWriteOperation(httpMethod) {
    ...
    if (httpMethod.toUpperCase() === 'PUT' || httpMethod.toUpperCase() === 'POST' || httpMethod.toUpperCase() === 'DELETE') {
      return true;
    } else {
      return false;
    }
