...
class SimplePrincipal {
  constructor(domain, name, creds, role, issueTime, authority) {
    this._domain = domain;
    this._name = name;
    this._creds = creds;
    this._roles = role;
  ...
  static createByUserIdentity(domain, name, creds, issueTime, authority) {
    ...
    if (domain !== null) {
      var matchDomain = (authority === null) ? null : authority.getDomain();
      if (matchDomain !== null && !(domain === matchDomain)) {
  ...
  getFullName() {
    if (this._fullName === null) {
      if (this._domain !== null && this._name !== null) {
