...
class Token {
  ...
  setTimeStamp(issueTime, expirationWindow) {
    this._timestamp = (issueTime > 0) ? issueTime : Date.now() / 1000;
    this._expiryTime = this._timestamp + expirationWindow;
  }

  validate(publicKey, allowedOffset, allowNoExpiry) {
    var err = null;
    if (this._unsignedToken === null || this._signature === null) {
    ...
    if (this._expiryTime !== 0 || !ATHENZ_TOKEN_NO_EXPIRY || !allowNoExpiry) {
