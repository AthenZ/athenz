...
func main() {
	...
	var grantType, subjectToken, subjectTokenType, requestedTokenType, audience, assertion string
	var expireTime int
	...
	flag.StringVar(&grantType, "grant-type", "", "grant type: token-exchange (for ID-JAG issue) or jwt-bearer (for JAG exchange)")
	flag.StringVar(&subjectToken, "subject-token", "", "file path to the subject token (ID token) for token exchange")
