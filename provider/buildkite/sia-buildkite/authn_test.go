...
func (f *customFormatter) FormatServiceURI(trustDomain, namespace, domain, service, workloadId string) string {
	return fmt.Sprintf("spiffe://custom.buildkite/svc/%s/%s", domain, service)
}
...
func TestGetCSRDetailsWithCustomSpiffeFormatter(t *testing.T) {
	...
	csr, err := GetCSRDetailsWithSpiffeFormatter(privateKey, "sports", "api", "sys.auth.build-kite", "0001", "athenz.io", "athenz", "", "", "", formatter)
	assert.Nil(t, err)

	block, _ := pem.Decode([]byte(csr))
	parsedCSR, err := x509.ParseCertificateRequest(block.Bytes)
	assert.Nil(t, err)
	assert.Equal(t, "spiffe://custom.buildkite/svc/sports/api", parsedCSR.URIs[0].String())
}
