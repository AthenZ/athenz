...
import (
	...
	"github.com/AthenZ/athenz/provider/gcp/sia-run/devel/metamock"
...
func TestGetConfigNoConfig(t *testing.T) {
	provider := GCPRunProvider{
		Name: fmt.Sprintf("test.gcp"),
	}
	config, err := GetRunConfig("devel/data/sia_empty_config", "http://127.0.0.1:5082", "us-west-2", provider)
	require.Nil(t, err)
...
func TestGetConfigNoServiceInConfig(t *testing.T) {
	...
	config, err := GetRunConfig("devel/data/sia_config_no_service", "http://127.0.0.1:5082", "us-west-2", provider)
...
func TestGetConfigWithConfig(t *testing.T) {
	...
	config, err := GetRunConfig("devel/data/sia_config", "http://127.0.0.1:5082", "us-west-2", provider)
	require.Nilf(t, err, "error should be empty, error: %v", err)
	require.NotNil(t, config, "should be able to get config")
