...
func main() {
	...
	gkeMetaEndPoint := flag.String("meta", "http://169.254.169.254:80", "meta endpoint")
	...
	config, accessProfileConfig, err := sia.GetGKEConfig(*pConf, *accessProfileConf, *gkeMetaEndPoint, region, provider)
	if err != nil {
	...
	opts.AddlSanDNSEntries = utils.GetK8SHostnames("cluster.local")
