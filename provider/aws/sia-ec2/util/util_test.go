...
func TestZtsHostName(test *testing.T) {
	...
	host := ZtsHostName("athenz.storage", "zts-aws-domain")
	if host != "storage.athenz.zts-aws-domain" {
		test.Errorf("Host is not expected storage.athenz.zts-aws-domain value")
		return
	}

	host = ZtsHostName("athenz.ci.storage", "zts-aws-domain")
	if host != "storage.athenz-ci.zts-aws-domain" {
		test.Errorf("Host is not expected storage.athenz-ci.zts-aws-domain value")
		return
	}

	host = ZtsHostName("athenz", "zts-aws-domain")
	if host != "..zts-aws-domain" {
		test.Errorf("Host is not expected ..zts-aws-domain value")
		return
	}
}
...
func TestGenerateCSR(test *testing.T) {
	...
	csr, err := GenerateCSR(key, "domain", "service", "domain.service", "", "Athenz", "spiffe://domain/sa/service", "zts-aws-domain", true)
	if err != nil {
	...
	if parsedcertreq.EmailAddresses[0] != "domain.service@zts-aws-domain" {
		test.Errorf("CSR does not have expected email address: %s", parsedcertreq.EmailAddresses[0])
		return
	}
	if parsedcertreq.DNSNames[0] != "service.domain.zts-aws-domain" {
...
func TestGenerateCSRWithDigitDomainNoSpiffe(test *testing.T) {
	...
	csr, err := GenerateCSR(key, "1v", "service", "domain.service", "", "Athenz", "spiffe://1v/sa/service", "zts-aws-domain", true)
	if err != nil {
		test.Errorf("Cannot create CSR: %v", err)
		return
	}
	...
	if parsedcertreq.URIs != nil {
		test.Errorf("CSR should not have a spiffe uri")
		return
	}
	if parsedcertreq.Subject.CommonName != "domain.service" {
		test.Errorf("CSR does not have expected common name: %s", parsedcertreq.Subject.CommonName)
