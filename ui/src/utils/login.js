  ...
  authenticateUser: function(app) {
    app.use(function(req, res, next) {
      if (req.originalUrl.startsWith(config.loginPath)) {
        ...
        if (req.body.token && req.authSvcToken) {
        ...
        // Otherwise we should redirect to login page
          ...
          return res.redirect(config.loginPath + '?target=' + q.escape(req.originalUrl));
        }

        // Authenticate user with auth_core
        var principal = authority.authenticate(req.authSvcToken, req.ip, req.method);
        if (!principal) {
          return res.redirect(config.loginPath + '?error=1&target=' + q.escape(req.originalUrl));
        }

        req.username = principal.getName() || req.config.user;
        req.user = {
