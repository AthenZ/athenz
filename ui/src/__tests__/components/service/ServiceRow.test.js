...
const fullServiceName = `${domainName}.${serviceName}`
describe('ServiceRow', () => {
    ...
    it('should render keys on click of key', async () => {
        ...
        expect(
            await waitFor(() => getByText(servicesForInitialState.services[fullServiceName]['description']))
        ).toMatchSnapshot();

        expect(
            await waitFor(() =>
                getByText('Public Key Version: ' + servicesForInitialState.services[fullServiceName].publicKeys['0']['id'])
            )
        ).toMatchSnapshot();

        expect(
            await waitFor(() =>
                getByText('Public Key Version: ' + servicesForInitialState.services[fullServiceName].publicKeys['1']['id'])
            )
        ).toMatchSnapshot();
    });
    ...
    it('should render providers on click of cloud', async () => {
        ...
        const api = {
            getServices: jest.fn().mockReturnValue(Promise.resolve([{name: fullServiceName}])),
