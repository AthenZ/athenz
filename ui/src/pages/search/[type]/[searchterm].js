...
class PageSearchDetails extends React.Component {
    ...
    setupServicesSearchResults() {
        ...
        let items = [];
        this.state.services.forEach(function (domainService) {
            const [domain, serviceName] =
                extractDomainAndService(domainService);
            items.push(
                <ResultsDiv key={domainService} data-wdio={'search-result'}>
                    <Link
                        ...
                        legacyBehavior
                    >
                        <StyledAnchor>{domainService}</StyledAnchor>
                    </Link>
                </ResultsDiv>
            );
        });
