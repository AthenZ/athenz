...
const MicrosegmentationsContainerDiv = styled.div`
    align-items: stretch;
    flex: 1 1;
...
const MicrosegmentationsContentDiv = styled.div``;
...
export async function getServerSideProps(context) {
    ...
    let notFound = false;
    let error = null;
    const microsegmentationsData = await Promise.all([api.getForm(), api.getPageFeatureFlag('microsegmentation')]).catch((err) => {
    ...
    return {
        props: {
            ...
            _csrf: microsegmentationsData[0],
            domainName: context.query.domain,
            nonce: context.req.headers.rid,
            pageFeatureFlag: microsegmentationsData[1],
        },
    };
}

class ServiceMicrosegmentationsPage extends React.Component {
    constructor(props) {
        super(props);
        this.api = API();
        ...
        return isLoading.includes('getDomainData') ? (
            ...
            <CacheProvider value={this.cache}>
                <div data-testid='service-microsegmentation'>
                    ...
                    <MainContentDiv>
                        <AppContainerDiv>
                            <MicrosegmentationsContainerDiv>
                                <MicrosegmentationsContentDiv>
                                    <PageHeaderDiv>
                                        <NameHeader
                                            category={'service'}
                                            domain={domainName}
                                        ...
                                        <ServiceTabs
                                            ...
                                            domain={domainName}
                                            service={serviceName}
                                            selectedName={'microsegmentations'}
                                        />
                                    </PageHeaderDiv>
                                    <RulesList
                                        api={this.api}
...
export default connect(mapStateToProps, mapDispatchToProps)(ServiceMicrosegmentationsPage);
