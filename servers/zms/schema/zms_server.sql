...
CREATE TABLE IF NOT EXISTS `zms_server`.`domain` (
  ...
  INDEX `idx_modified` (`modified` ASC) VISIBLE,
  INDEX `idx_account` (`account` ASC) VISIBLE,
  INDEX `idx_ypmid` (`ypm_id` ASC) VISIBLE,
  INDEX `idx_appid` (`application_id` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `zms_server`.`role` (
  ...
  `sign_algorithm` VARCHAR(64) NOT NULL DEFAULT '',
  PRIMARY KEY (`role_id`),
  UNIQUE INDEX `uq_domain_role` (`domain_id` ASC, `name` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `zms_server`.`policy` (
  ...
  INDEX `fk_policy_domain_idx` (`domain_id` ASC) VISIBLE,
  UNIQUE INDEX `uq_domain_policy` (`name` ASC, `domain_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `zms_server`.`assertion` (
  ...
  INDEX `fk_assertion_policy_idx` (`policy_id` ASC) VISIBLE,
  INDEX `idx_role` (`role` ASC) VISIBLE,
  INDEX `idx_resource` (`resource` ASC) VISIBLE,
  INDEX `idx_action` (`action` ASC) VISIBLE,
  INDEX `idx_effect` (`effect` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `zms_server`.`service` (
  ...
  INDEX `fk_service_domain_idx` (`domain_id` ASC) VISIBLE,
  UNIQUE INDEX `uq_domain_service` (`name` ASC, `domain_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `zms_server`.`service_host` (
  ...
  INDEX `idx_host` (`host_id` ASC, `service_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `zms_server`.`domain_template` (
  ...
  UNIQUE INDEX `uq_domain_template` (`template` ASC, `domain_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `zms_server`.`role_audit_log` (
  ...
  INDEX `fk_role_audit_log_role_id_idx` (`role_id` ASC) VISIBLE,
