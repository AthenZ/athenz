...
public class ServiceProviderClient {
    ...
    public ServiceProviderClient(PrivateKeyStore keyStore, String homeDomainPrefix) throws KeyRefresherException, IOException, InterruptedException {
    ...
    public DomainDependencyProviderResponse getDependencyStatus(String service, String endpoint, boolean isInstanceProvider, String domain, String principal) {
        ...
        try {
            return getDependencyStatusFromProvider(service, endpoint, isInstanceProvider, domain, principal);
        } catch (Exception ex) {
    ...
    private DomainDependencyProviderResponse getDependencyStatusFromProvider(String service, String endpoint, boolean isInstanceProvider, String domain, String principal) throws IOException {
        DomainDependencyProviderResponse domainDependencyProviderResponse = new DomainDependencyProviderResponse();
        String url = getProviderEndpoint(endpoint, isInstanceProvider);
        ...
        if (httpResponse.getStatusCode() >= 500 && httpResponse.getStatusCode() <= 599) {
            ...
            String errorMessage = String.format("Invalid dependency status request to service povider '%s' with endpoint '%s', Received error code %d", service, url, httpResponse.getStatusCode());
    ...
    private String getProviderEndpoint(String endpoint, boolean isInstanceProvider) {
        String url = endpoint;
