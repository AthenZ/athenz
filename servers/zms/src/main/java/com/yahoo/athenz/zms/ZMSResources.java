...
public class ZMSResources {
    ...
    @GET
    @Path("/dependency/domain/{domainName}/resourceGroup/service/{service}")
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(description = "List registered resource groups for domain and service")
    public DependentServiceResourceGroup getDependentServiceResourceGroup(
        @Parameter(description = "name of the domain", required = true) @PathParam("domainName") String domainName,
        @Parameter(description = "name of the service", required = true) @PathParam("service") String service) {
        int code = ResourceException.OK;
        ResourceContext context = null;
        try {
            context = this.delegate.newResourceContext(this.request, this.response, "getDependentServiceResourceGroup");
            context.authenticate();
            return this.delegate.getDependentServiceResourceGroup(context, domainName, service);
        } catch (ResourceException e) {
            code = e.getCode();
            switch (code) {
            case ResourceException.BAD_REQUEST:
            ...
            case ResourceException.UNAUTHORIZED:
                throw typedException(code, e, ResourceError.class);
            default:
                System.err.println("*** Warning: undeclared exception (" + code + ") for resource getDependentServiceResourceGroup");
                throw typedException(code, e, ResourceError.class);
            }
        } finally {
            this.delegate.recordMetrics(context, code);
