...
public class CertFailedRefreshNotificationTask implements NotificationTask {
    ...
    private final static String SNOOZED_DOMAIN_TAG_KEY = "DisableCertRefreshNotification";
    ...
    private boolean isDomainSnoozed(DomainData domainData) {
        Map<String, StringList> domainTags = domainData.getTags();
        if (domainTags == null || domainTags.get(SNOOZED_DOMAIN_TAG_KEY) == null || domainTags.get(SNOOZED_DOMAIN_TAG_KEY).getList() == null) {
            return false;
        }

        List<String> snoozeTagValues = domainTags.get(SNOOZED_DOMAIN_TAG_KEY).getList();
