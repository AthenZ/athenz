    ...
    public void testSomeRecordsSnoozed() {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            if (i % 2 == 0) {
                ...
                tags.put("DisableCertRefreshNotification", new StringList().setList(Arrays.asList("true")));
                domainData.setTags(tags);
            }

            Mockito.when(dataStore.getDomainData("domain" + i)).thenReturn(domainData);
            Mockito.when(hostnameResolver.isValidHostname(eq("hostName" + i))).thenReturn(true);
        }

        // Have DisableCertRefreshNotification tag for domain 8 but with value other then "true"
        X509CertRecord record = getMockX509CertRecord(currentDate, 8);
        records.add(record);
        NotificationTestsCommon.mockDomainData(8, dataStore);
        DomainData domainData = new DomainData();
        Map<String, StringList> tags = new HashMap<>();
        tags.put("DisableCertRefreshNotification", new StringList().setList(Arrays.asList("false", "False","Not True")));
        domainData.setTags(tags);
        Mockito.when(dataStore.getDomainData("domain" + 8)).thenReturn(domainData);
        Mockito.when(hostnameResolver.isValidHostname(eq("hostName" + 8))).thenReturn(true);

        // Have DisableCertRefreshNotification tag for domain 9 with several values (one of them is true case insensitive)
        record = getMockX509CertRecord(currentDate, 9);
        records.add(record);
        NotificationTestsCommon.mockDomainData(9, dataStore);
        domainData = new DomainData();
        tags = new HashMap<>();
        tags.put("DisableCertRefreshNotification", new StringList().setList(Arrays.asList("false", "test", "tRue")));
        domainData.setTags(tags);
        Mockito.when(dataStore.getDomainData("domain" + 9)).thenReturn(domainData);
