...
describe('zts_client impl', function() {
  ...
  it('should test _initClient: domainName or serviceName or siaProvidr be null: result error', function() {
    ...
    try {
      ...
      expect(e.message).to.contain('domainName & serviceName & siaProvider must be specified.');
      return;
    }
    expect(1).to.be.false;
  });
  ...
  it('should test _updateServicePrincipal: siaProvider.getIdentity null: result error', function() {
    ...
    try {
      ...
      expect(e.message).to.contain('UpdateServicePrincipal: Unable to get PrincipalToken');
      return;
    }
    expect(1).to.be.false;
  });
  ...
  it('should test getRoleToken: get cache', function() {
    ...
    var ck = 'p=' + domainName + '.' + serviceName + ';d=' + paramsR.domainName + ';r=' + paramsR.roleName + ';u=' + paramsR.proxyForPrincipal;
    cache.del(ck);
  ...
  it('should test getRoleToken: domainName Null', function() {
    ...
    try {
      ...
      expect(e.message).to.contain('GetRoleToken: domainName must not be null.');
      return;
    }
    expect(1).to.be.false;
  });
  ...
  it('should test _getRoleTokenCacheKeySetTenant', function() {
    ...
    expect(zts._getRoleTokenCacheKeySetTenant(paramsR.domainName, paramsR.roleName, paramsR.proxyForPrincipal)).to.equal('p=' + domainName + '.' + serviceName + ';d=' + paramsR.domainName + ';r=' + paramsR.roleName + ';u=' + paramsR.proxyForPrincipal);
  });

  it('should test _getRoleTokenCacheKey: tenantDomain null: result null', function() {
    var zts = new ztsClient(params);

    expect(zts._getRoleTokenCacheKey(null, serviceName, paramsR.domainName, paramsR.roleName, paramsR.proxyForPrincipal)).to.be.null;
  });

  it('should test _getRoleTokenCacheKey: tenantService null', function() {
    var zts = new ztsClient(params);

    expect(zts._getRoleTokenCacheKey(domainName, null, paramsR.domainName, paramsR.roleName, paramsR.proxyForPrincipal)).to.equal('p=' + domainName + ';d=' + paramsR.domainName + ';r=' + paramsR.roleName + ';u=' + paramsR.proxyForPrincipal);
  });

  it('should test _getRoleTokenCacheKey: roleName null', function() {
    var zts = new ztsClient(params);

    expect(zts._getRoleTokenCacheKey(domainName, serviceName, paramsR.domainName, null, paramsR.proxyForPrincipal)).to.equal('p=' + domainName + '.' + serviceName + ';d=' + paramsR.domainName + ';u=' + paramsR.proxyForPrincipal);
  });

  it('should test _getRoleTokenCacheKey: proxyForPrincipal null', function() {
    var zts = new ztsClient(params);

    expect(zts._getRoleTokenCacheKey(domainName, serviceName, paramsR.domainName, paramsR.roleName, null)).to.equal('p=' + domainName + '.' + serviceName + ';d=' + paramsR.domainName + ';r=' + paramsR.roleName);
  });
