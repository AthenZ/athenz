...
func init() {
	...
	rPutUserMeta := rdl.NewResourceBuilder("UserMeta", "PUT", "/user/{name}/meta")
	rPutUserMeta.Comment("Update the specified user domain(s) metadata. Note that entities in the domain are not affected. This API is only available to system administrators in order to disable user accounts when the user is no longer active, for example, before eventual deletion of the account after some grace period")
	rPutUserMeta.Input("name", "SimpleName", true, "", "", false, nil, "name of the user")
	rPutUserMeta.Input("auditRef", "String", false, "", "Y-Audit-Ref", false, nil, "Audit reference")
	rPutUserMeta.Input("detail", "UserMeta", false, "", "", false, nil, "UserMeta object with updated attribute values")
	rPutUserMeta.Auth("update", "sys.auth:user", false, "")
	rPutUserMeta.Expected("NO_CONTENT")
	rPutUserMeta.Exception("BAD_REQUEST", "ResourceError", "")
	...
	rDeleteUser.Input("auditRef", "String", false, "", "Y-Audit-Ref", false, nil, "Audit reference")
