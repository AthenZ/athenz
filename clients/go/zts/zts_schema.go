...
func init() {
	...
	tExternalMemberName.Comment("External Member name. Even though the pattern is open to support all possible external members, the server will validate that it does not include * character")
	...
	tInstanceRegisterInformation.Field("attestationData", "String", true, nil, "identity attestation data including document with its signature containing attributes like IP address, instance-id, account#, etc.")
	tInstanceRegisterInformation.Field("csr", "String", true, nil, "the Certificate Signing Request for the expected X.509 certificate in the response")
	...
	tInstanceRegisterInformation.Field("jwtSVIDSpiffe", "String", true, nil, "the spiffe uri for the jwt svid")
	tInstanceRegisterInformation.Field("jwtSVIDSpiffeSubject", "Bool", true, nil, "if true, return the spiffe uri as the jwt svid sub claim value")
	tInstanceRegisterInformation.Field("jwtSVIDKeyType", "SimpleName", true, nil, "optional signing key type - RSA or EC. Might be ignored if server doesn't have the requested type configured")
	...
	mGetResourceAccess.Input("checkPrincipal", "PrincipalName", false, "principal", "", true, nil, "usually null. If present, carry out the access check for this principal")
	...
	mGetResourceAccessExt.Input("checkPrincipal", "PrincipalName", false, "principal", "", true, nil, "usually null. If present, carry out the access check for this principal")
	...
	mGetAccess.Input("principal", "PrincipalName", true, "", "", false, nil, "carry out the access check for this principal")
	...
	mGetRoleAccess.Input("principal", "PrincipalName", true, "", "", false, nil, "carry out the role access lookup for this principal")
