...
func init() {
	...
	mPostAWSInstanceInformation := rdl.NewResourceBuilder("Identity", "POST", "/aws/instance")
	mPostAWSInstanceInformation.Comment("Register an instance in AWS ZTS. Whether this succeeds or not depends on the contents of the request (the request itself is not authenticated or authorized in the normal way). If successful, the Identity is returned as a x.509 client certificate (to be used in TLS operations)")
	mPostAWSInstanceInformation.Input("info", "AWSInstanceInformation", false, "", "", false, nil, "")
	mPostAWSInstanceInformation.Exception("BAD_REQUEST", "ResourceError", "")
	mPostAWSInstanceInformation.Exception("FORBIDDEN", "ResourceError", "")
	mPostAWSInstanceInformation.Exception("UNAUTHORIZED", "ResourceError", "")
	sb.AddResource(mPostAWSInstanceInformation.Build())

	mPostAWSCertificateRequest := rdl.NewResourceBuilder("Identity", "POST", "/aws/instance/{domain}/{service}/refresh")
	mPostAWSCertificateRequest.Comment("Rotate certs. Make this request with previous cert, the result are new certs for the same identity.")
	mPostAWSCertificateRequest.Input("domain", "CompoundName", true, "", "", false, nil, "name of the domain requesting the refresh")
	mPostAWSCertificateRequest.Input("service", "SimpleName", true, "", "", false, nil, "name of the service requesting the refresh")
	mPostAWSCertificateRequest.Input("req", "AWSCertificateRequest", false, "", "", false, nil, "the refresh request")
	mPostAWSCertificateRequest.Auth("", "", true, "")
	mPostAWSCertificateRequest.Exception("BAD_REQUEST", "ResourceError", "")
	mPostAWSCertificateRequest.Exception("FORBIDDEN", "ResourceError", "")
	mPostAWSCertificateRequest.Exception("UNAUTHORIZED", "ResourceError", "")
	sb.AddResource(mPostAWSCertificateRequest.Build())
